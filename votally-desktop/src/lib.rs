use iced;
use iced::widget::{
    Column, button, checkbox, container, radio, rich_text, scrollable, span, text, text_input,
};
use iced::{Element, Length, Task, never};
use std::sync::Arc;
    ...
    pub fn view(&self) -> Element<'_, Message> {
        match self.client {
            ...
            Some(_) => match &self.info {
                None => container_centered(text(if self.sending_vote {
                    "Sending ballot ..."
                } else {
                    "Waiting info"
                }))
                .into(),
                Some(i) => container_centered(
                    Column::from_vec(vec![
                        rich_text![span(i.get_name()).underline(true)]
                            .on_link_click(never)
                            .into(),
                        container(
