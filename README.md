...
`libvotally` contains the core voting logic.
`votally-cli` is thin wrapper exposing a CLI.
`votally-desktop` is GUI app.


## Build
The following commands build the Votally CLI and GUI.
The generated binaries can be found respectively at `target/release/votally-cli` and at `target/release/votally-desktop`.
```bash
cargo build --release
```
